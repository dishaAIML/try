...
st.set_page_config(
    ...
    page_icon='./Logo/resume_analyzer.ico'
)
...
def run():
    ...
    link = '[@Developed by Disha](https://github.com/dishaAIML)'
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            with st.spinner('Uploading your Resume....'):
                time.sleep(4)
            os.makedirs("Uploaded_Resumes", exist_ok=True)  # auto-create folder
            ...
            if resume_data:
                ...
                # Function to fetch the title of a YouTube video using pafy
                def fetch_yt_video(link):
                    try:
                        video = pafy.new(link)
                        return video.title
                    except Exception as e:
                        return f"Error fetching video title: {e}"
